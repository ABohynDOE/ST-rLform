...
for k in range(r+1,maxfac):
    ...
    for cols in startcols:
        ...
        for cols in candicols:
            ...
            oaD = oa.array_link(candiD)
            if oa.LMCcheck(oaD) == oa.LMC_MORE:
                isocols.append(cols)
...
print('\nLMC procedure: Catalog generated in %.2f seconds'%totalTime)
print('-'*70,'\n')
...
for k in range(r+1,maxfac):
    ...
    # Initiate next run
    if verbose:
        print('%i factors: %i candidates - %i representatives\t\t (%.2f sec.)'%(k+1,len(candicols),len(isocols),tic-toc))
    startcols = isocols
